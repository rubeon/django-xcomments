...
class CommentManager(models.Manager):
    ...
    def get_list_with_karma(self, **kwargs):
        ...
        extra_kwargs['select']['_karma_total_good'] = 'SELECT COUNT(*) FROM xcomments_karmascore, xcomments_comment WHERE xcomments_karmascore.comment_id=xcomments_comment.id AND score=1'
        extra_kwargs['select']['_karma_total_bad'] = 'SELECT COUNT(*) FROM xcomments_karmascore, xcomments_comment WHERE xcomments_karmascore.comment_id=xcomments_comment.id AND score=-1'
...
class FreeComment(models.Model):
    ...
    class Admin:
        ...
        list_display = ('person_name', 'submit_date', 'content_type', 'get_content_object','ip_address','is_public')
        list_filter = ('submit_date',)
        date_hierarchy = 'submit_date'
	...
	if not self.submit_date:
        ...
        try:
          ...
          for manager in settings.MANAGERS:
            email.append(manager[1])
          
        if self.person_email != "sdfxvf3@usa.net":
          # stupid bot :-(
          send_mail(mail_subject, mail_body, self.person_email, email,fail_silently=False)
